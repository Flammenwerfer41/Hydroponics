...
CacheBucket* historySnapshot = nullptr;
LightEvent lightEventCache[MAX_LIGHT_EVENTS]{};
LightEvent* lightEventSnapshot = nullptr;

constexpr size_t HISTORY_SNAPSHOT_BYTES =
  HISTORY_SNAPSHOT_SIZE * sizeof(CacheBucket);
constexpr size_t LIGHT_EVENT_SNAPSHOT_BYTES =
  MAX_LIGHT_EVENTS * sizeof(LightEvent);
...
void setup() {
  ...
  historySnapshot = new (std::nothrow) CacheBucket[HISTORY_SNAPSHOT_SIZE]{};
  lightEventSnapshot = new (std::nothrow) LightEvent[MAX_LIGHT_EVENTS]{};
  if (!historySnapshot || !lightEventSnapshot) {
    Serial.println("FATAL: snapshot buffer allocation failed; execution stopped.");
  ...
  Serial.printf("RAM cache bytes: history=%u, light=%u, snapshots=%u, free heap before tasks=%u\n",
                ...
                static_cast<unsigned>(HISTORY_SNAPSHOT_BYTES + LIGHT_EVENT_SNAPSHOT_BYTES +
