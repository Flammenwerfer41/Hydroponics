...
- 완료: SCD40 CO₂와 VEML7700 조도 센서의 전원·I2C 하드웨어 검증과 운영 배치
- 완료: `co2_concentration`·`illuminance` 품질, 저장, 조회와 대시보드 확장
- 완료: 버전이 지정된 식물등 계수로 추정 PPFD 표시와 일일 보고서 브리지 확장
- 완료: CO₂·조도 결측 및 지속적인 고CO₂ 상태의 D1·Discord 경고
...
- 추정값이라는 한정어와 계수 버전을 숨긴 채 조도 환산값을 실측 PPFD로 표시하는 기능
