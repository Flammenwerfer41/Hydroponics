/*
  ESP32 + BME280 Hydroponics Environment Logger v7.0
  ------------------------------------------------
  Features
...
bool scanLightEventFile(){newestLightEventSlot=-1;validLightEventCount=0;File f=LittleFS.open(LIGHT_EVENT_FILE_PATH,"r");if(!f)return false;uint32_t newestTs=0;LightEvent newest{};for(uint32_t slot=0;slot<MAX_LIGHT_EVENTS;slot++){LightEvent e{};if(!readLightEventAt(f,slot,e)){f.close();return false;}if(e.timestamp>=VALID_EPOCH_MIN&&e.state<=1){validLightEventCount++;if(e.timestamp>=newestTs){newestTs=e.timestamp;newestLightEventSlot=(int32_t)slot;newest=e;}}}f.close();if(newestLightEventSlot>=0){latestLightStateKnown=true;latestLightOn=newest.state==1;}Serial.printf("Light event scan: %u events, newest slot %ld\n",validLightEventCount,(long)newestLightEventSlot);return true;}
...
bool appendLightEvent(uint32_t timestamp,bool on){if(!filesystemReady||timestamp<VALID_EPOCH_MIN)return false;if(!takeMutex(fsMutex,pdMS_TO_TICKS(1000)))return false;uint32_t next=newestLightEventSlot<0?0:((uint32_t)newestLightEventSlot+1UL)%MAX_LIGHT_EVENTS;File f=LittleFS.open(LIGHT_EVENT_FILE_PATH,"r+");if(!f){giveMutex(fsMutex);return false;}LightEvent e{};e.timestamp=timestamp;e.state=on?1:0;bool ok=writeLightEventAt(f,next,e);f.flush();f.close();if(ok){newestLightEventSlot=(int32_t)next;if(validLightEventCount<MAX_LIGHT_EVENTS)validLightEventCount++;}giveMutex(fsMutex);if(ok)Serial.printf("Light event: %s at %s\n",on?"ON":"OFF",formatLocalTime(timestamp).c_str());return ok;}
...
void sendLightIntervalsJson(const RangeSettings& s){bool first=true;if(!filesystemReady||validLightEventCount==0)return;if(!takeMutex(fsMutex,pdMS_TO_TICKS(1000)))return;File f=LittleFS.open(LIGHT_EVENT_FILE_PATH,"r");if(!f){giveMutex(fsMutex);return;}uint32_t startSlot=oldestLightEventSlot();bool stateKnown=false,stateOn=false;uint32_t onStart=0,seen=0;for(uint32_t i=0;i<MAX_LIGHT_EVENTS&&seen<validLightEventCount;i++){uint32_t slot=(startSlot+i)%MAX_LIGHT_EVENTS;LightEvent e{};if(!readLightEventAt(f,slot,e))break;if(e.timestamp<VALID_EPOCH_MIN||e.state>1)continue;seen++;if(e.timestamp<=(uint32_t)s.start){stateKnown=true;stateOn=e.state==1;if(stateOn)onStart=(uint32_t)s.start;continue;}if(e.timestamp>=(uint32_t)s.end)break;if(!stateKnown){stateKnown=true;stateOn=e.state==1;if(stateOn)onStart=e.timestamp;continue;}bool nextOn=e.state==1;if(stateOn&&!nextOn){char line[64];snprintf(line,sizeof(line),"%s[%u,%u]",first?"":",",onStart,e.timestamp);server.sendContent(line);first=false;}else if(!stateOn&&nextOn)onStart=e.timestamp;stateOn=nextOn;}if(stateKnown&&stateOn){char line[64];snprintf(line,sizeof(line),"%s[%u,%u]",first?"":",",onStart,(uint32_t)s.end);server.sendContent(line);}f.close();giveMutex(fsMutex);}

void handleRoot(){server.send_P(200,"text/html; charset=utf-8",DASHBOARD_HTML);}
void handleCurrent(){time_t now;time(&now);float t,h,p,lv,lp,lc;int rssi,cloudCode,sbHttp,sbStatus;time_t measured,sbUpdated;uint32_t failureStreak,sbFailures,lightMinutes;bool lightKnown,lightOn;if(stateMutex)xSemaphoreTake(stateMutex,portMAX_DELAY);t=latestTemperature;h=latestHumidity;p=latestPressure;rssi=latestRssi;cloudCode=latestThingSpeakCode;measured=latestMeasurementTime;failureStreak=consecutiveUploadFailures;lightKnown=latestLightStateKnown;lightOn=latestLightOn;lv=latestLightVoltage;lp=latestLightPower;lc=latestLightCurrentA;lightMinutes=latestLightMinutesToday;sbUpdated=latestSwitchBotUpdateTime;sbHttp=latestSwitchBotHttpCode;sbStatus=latestSwitchBotStatusCode;sbFailures=consecutiveSwitchBotFailures;if(stateMutex)xSemaphoreGive(stateMutex);String j;j.reserve(900);j+="{";j+="\"temperature\":";j+=isfinite(t)?String(t,2):"null";j+=",\"humidity\":";j+=isfinite(h)?String(h,2):"null";j+=",\"pressure\":";j+=isfinite(p)?String(p,2):"null";j+=",\"rssi\":"+String(WiFi.status()==WL_CONNECTED?WiFi.RSSI():rssi);j+=",\"uptimeText\":\""+uptimeText()+"\"";j+=",\"cloudOk\":";j+=cloudCode==200?"true":"false";j+=",\"cloudCode\":"+String(cloudCode);j+=",\"measurementTime\":\""+formatLocalTime(measured)+"\"";j+=",\"deviceTime\":\""+formatLocalTime(now)+"\"";j+=",\"lightKnown\":";j+=lightKnown?"true":"false";j+=",\"lightOn\":";j+=lightOn?"true":"false";j+=",\"lightVoltage\":";j+=isfinite(lv)?String(lv,1):"null";j+=",\"lightPower\":";j+=isfinite(lp)?String(lp,1):"null";j+=",\"lightCurrent\":";j+=isfinite(lc)?String(lc,3):"null";j+=",\"lightRuntimeText\":\""+lightRuntimeText(lightMinutes)+"\"";j+=",\"switchBotOk\":";j+=(sbHttp==200&&sbStatus==100)?"true":"false";j+=",\"switchBotHttpCode\":"+String(sbHttp);j+=",\"switchBotStatusCode\":"+String(sbStatus);j+=",\"switchBotUpdateTime\":\""+formatLocalTime(sbUpdated)+"\"";j+=",\"switchBotFailures\":"+String(sbFailures);j+=",\"records\":"+String(validRecordCount);j+=",\"lightEvents\":"+String(validLightEventCount);j+=",\"cloudFailureStreak\":"+String(failureStreak);j+=",\"uploadQueue\":"+String(thingSpeakQueue?uxQueueMessagesWaiting(thingSpeakQueue):0);j+=",\"droppedUploads\":"+String(droppedUploadJobs);j+=",\"cacheReady\":";j+=historyCacheReady?"true":"false";j+="}";server.send(200,"application/json; charset=utf-8",j);}
...
void setup(){Serial.begin(115200);delay(800);Serial.println("\nESP32 hydroponics logger v7.0 starting.");fsMutex=xSemaphoreCreateMutex();stateMutex=xSemaphoreCreateMutex();if(!fsMutex||!stateMutex)Serial.println("WARNING: mutex creation failed.");Wire.begin(I2C_SDA_PIN,I2C_SCL_PIN);initializeBME280();connectWiFi();if(WiFi.status()==WL_CONNECTED){requestTimeSync();setupOTA();setupMDNS();}thingSpeakClient.setTimeout(3000);ThingSpeak.begin(thingSpeakClient);initializeFilesystem();rebuildHistoryCaches();Serial.printf("RAM cache bytes: %u, free heap before task: %u bytes\n",(unsigned)(sizeof(dayCache)+sizeof(weekCache)+sizeof(monthCache)),ESP.getFreeHeap());setupThingSpeakTask();setupSwitchBotTask();setupWebServer();lastSampleMs=millis()-SAMPLE_INTERVAL_MS;Serial.println("Setup complete.");}
void loop(){maintainWiFi();if(WiFi.status()==WL_CONNECTED){maintainTimeSync();if(!otaReady)setupOTA();if(!mdnsReady)setupMDNS();}serviceNetwork();if(millis()-lastSampleMs>=SAMPLE_INTERVAL_MS){lastSampleMs=millis();performMeasurementCycle();}delay(5);}
