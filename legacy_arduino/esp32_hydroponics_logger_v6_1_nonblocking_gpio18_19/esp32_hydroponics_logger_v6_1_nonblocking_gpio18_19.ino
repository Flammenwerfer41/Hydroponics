/*
  ESP32 + BME280 Hydroponics Environment Logger v6.1
  ------------------------------------------------
  Features
...
constexpr uint32_t MONTH_BUCKET_SECONDS = 1800UL;
...
let activeRange="day",historyData={start:0,end:1,points:[]},historyLoading=false,pendingHistoryRange=null;const $=id=>document.getElementById(id);const fmt=(v,n=1)=>Number.isFinite(v)?v.toFixed(n):"--";
async function refreshCurrent(){try{const r=await fetch('/api/current',{cache:'no-store'}),d=await r.json();$('temp').textContent=fmt(d.temperature,2);$('hum').textContent=fmt(d.humidity,2);$('pres').textContent=fmt(d.pressure,2);$('rssi').textContent=d.rssi??'--';$('uptime').textContent=d.uptimeText||'--';$('cloud').textContent=d.cloudOk?'정상':(d.cloudCode===0?'대기':('오류 '+d.cloudCode));$('clock').textContent='마지막 측정: '+(d.measurementTime||'--')+' · 장치 시각: '+(d.deviceTime||'--')}catch(e){$('clock').textContent='현재 상태를 불러오지 못했습니다.'}}
...
void clearCacheBucket(CacheBucket& bucket,uint32_t bucketStart){bucket.bucketStart=bucketStart;bucket.count=0;bucket.reserved=0;bucket.sumT=0;bucket.sumH=0;bucket.sumP=0;bucket.sumRssi=0;}
template<size_t N>void addToCache(CacheBucket (&cache)[N],uint32_t bucketSeconds,const SensorRecord& r){uint32_t bucketStart=(r.timestamp/bucketSeconds)*bucketSeconds;size_t index=(bucketStart/bucketSeconds)%N;CacheBucket& b=cache[index];if(b.bucketStart!=bucketStart)clearCacheBucket(b,bucketStart);if(b.count<UINT16_MAX)b.count++;b.sumT+=r.temperature;b.sumH+=r.humidity;b.sumP+=r.pressure;b.sumRssi+=r.rssi;}
...
template<size_t N>void sendCacheJson(CacheBucket (&cache)[N],const RangeSettings& s){bool first=true;for(uint32_t ts=(uint32_t)s.start-(uint32_t)s.start%s.bucketSeconds;ts<(uint32_t)s.end;ts+=s.bucketSeconds){size_t index=(ts/s.bucketSeconds)%N;CacheBucket b{};if(stateMutex)xSemaphoreTake(stateMutex,portMAX_DELAY);b=cache[index];if(stateMutex)xSemaphoreGive(stateMutex);if(b.bucketStart!=ts||b.count==0)continue;uint32_t pointTs=ts+s.bucketSeconds/2;char line[128];snprintf(line,sizeof(line),"%s[%u,%.3f,%.3f,%.3f,%.1f]",first?"":",",pointTs,b.sumT/b.count,b.sumH/b.count,b.sumP/b.count,b.sumRssi/b.count);server.sendContent(line);first=false;yield();}}

void handleRoot(){server.send_P(200,"text/html; charset=utf-8",DASHBOARD_HTML);}
void handleCurrent(){time_t now;time(&now);float t,h,p;int rssi,cloudCode;time_t measured;uint32_t failureStreak;if(stateMutex)xSemaphoreTake(stateMutex,portMAX_DELAY);t=latestTemperature;h=latestHumidity;p=latestPressure;rssi=latestRssi;cloudCode=latestThingSpeakCode;measured=latestMeasurementTime;failureStreak=consecutiveUploadFailures;if(stateMutex)xSemaphoreGive(stateMutex);String j;j.reserve(520);j+="{";j+="\"temperature\":";j+=isfinite(t)?String(t,2):"null";j+=",\"humidity\":";j+=isfinite(h)?String(h,2):"null";j+=",\"pressure\":";j+=isfinite(p)?String(p,2):"null";j+=",\"rssi\":"+String(WiFi.status()==WL_CONNECTED?WiFi.RSSI():rssi);j+=",\"uptimeText\":\""+uptimeText()+"\"";j+=",\"cloudOk\":";j+=cloudCode==200?"true":"false";j+=",\"cloudCode\":"+String(cloudCode);j+=",\"measurementTime\":\""+formatLocalTime(measured)+"\"";j+=",\"deviceTime\":\""+formatLocalTime(now)+"\"";j+=",\"records\":"+String(validRecordCount);j+=",\"cloudFailureStreak\":"+String(failureStreak);j+=",\"uploadQueue\":"+String(thingSpeakQueue?uxQueueMessagesWaiting(thingSpeakQueue):0);j+=",\"droppedUploads\":"+String(droppedUploadJobs);j+=",\"cacheReady\":";j+=historyCacheReady?"true":"false";j+="}";server.send(200,"application/json; charset=utf-8",j);}
void handleHistory(){if(!historyCacheReady){server.send(503,"application/json","{\"error\":\"History cache is still building\"}");return;}time_t now;time(&now);if(!isTimeValid(now)){server.send(503,"application/json","{\"error\":\"Time not synchronized\"}");return;}String range=server.hasArg("range")?server.arg("range"):"day";if(range!="day"&&range!="week"&&range!="month")range="day";RangeSettings s=historyRange(range,now);server.setContentLength(CONTENT_LENGTH_UNKNOWN);server.send(200,"application/json; charset=utf-8","");char head[128];snprintf(head,sizeof(head),"{\"range\":\"%s\",\"start\":%u,\"end\":%u,\"points\":[",range.c_str(),(uint32_t)s.start,(uint32_t)s.end);server.sendContent(head);if(range=="day")sendCacheJson(dayCache,s);else if(range=="week")sendCacheJson(weekCache,s);else sendCacheJson(monthCache,s);server.sendContent("]}");server.sendContent("");}
