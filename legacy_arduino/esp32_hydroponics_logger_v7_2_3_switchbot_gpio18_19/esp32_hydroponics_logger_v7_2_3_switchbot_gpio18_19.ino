/*
  ESP32 + BME280 Hydroponics Environment Logger v7.2.3
  ------------------------------------------------
  Features
  ...
  - Dashboard uses completion-based refresh loops instead of setInterval
  - Background tabs pause API polling and refresh once when visible again
  - Current/history requests cannot overlap; stalled fetches are aborted by timeout
*/
...
let activeRange="day",historyData={start:0,end:1,bucketSeconds:120,points:[],lightIntervals:[]};
let currentLoading=false,historyLoading=false,currentTimer=null,historyTimer=null;
let currentController=null,historyController=null;
const CURRENT_REFRESH_MS=15000,HISTORY_REFRESH_MS=120000,FETCH_TIMEOUT_MS=20000;
const $=id=>document.getElementById(id);const fmt=(v,n=1)=>Number.isFinite(v)?v.toFixed(n):"--";
function scheduleCurrent(delay=CURRENT_REFRESH_MS){clearTimeout(currentTimer);currentTimer=setTimeout(currentRefreshLoop,delay)}
function scheduleHistory(delay=HISTORY_REFRESH_MS){clearTimeout(historyTimer);historyTimer=setTimeout(historyRefreshLoop,delay)}
function abortFetch(controller){if(controller)controller.abort()}
async function refreshCurrent(){if(currentLoading||document.hidden)return;currentLoading=true;currentController=new AbortController();const timeout=setTimeout(()=>currentController.abort(),FETCH_TIMEOUT_MS);try{const r=await fetch('/api/current',{cache:'no-store',signal:currentController.signal});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();$('temp').textContent=fmt(d.temperature,2);$('hum').textContent=fmt(d.humidity,2);$('pres').textContent=fmt(d.pressure,2);$('rssi').textContent=d.rssi??'--';$('uptime').textContent=d.uptimeText||'--';$('cloud').textContent=d.cloudOk?'정상':(d.cloudCode===0?'대기':('오류 '+d.cloudCode));$('lightState').textContent=d.lightKnown?(d.lightOn?'ON':'OFF'):'--';$('lightPower').textContent=fmt(d.lightPower,1);$('lightVoltage').textContent=fmt(d.lightVoltage,1);$('lightCurrent').textContent=fmt(d.lightCurrent,3);$('lightRuntime').textContent=d.lightRuntimeText||'--';$('switchbot').textContent=d.switchBotOk?('정상 · '+(d.switchBotUpdateTime||'--')):(d.switchBotHttpCode===0?'대기':('오류 '+d.switchBotHttpCode));$('clock').textContent='마지막 측정: '+(d.measurementTime||'--')+' · 장치 시각: '+(d.deviceTime||'--')}catch(e){if(e.name!=='AbortError')$('clock').textContent='현재 상태를 불러오지 못했습니다.'}finally{clearTimeout(timeout);currentController=null;currentLoading=false}}
async function loadHistory(range){if(historyLoading||document.hidden)return false;historyLoading=true;activeRange=range;document.querySelectorAll('button[data-range]').forEach(b=>b.classList.toggle('active',b.dataset.range===range));$('state').textContent='데이터 불러오는 중…';historyController=new AbortController();const timeout=setTimeout(()=>historyController.abort(),FETCH_TIMEOUT_MS);try{const r=await fetch('/api/history?range='+encodeURIComponent(range),{cache:'no-store',signal:historyController.signal});if(!r.ok)throw new Error('HTTP '+r.status);historyData=await r.json();$('state').textContent=historyData.points.length.toLocaleString()+'개 점';drawAll();return true}catch(e){if(e.name==='AbortError')$('state').textContent='그래프 요청 시간 초과';else $('state').textContent='그래프 데이터 오류';return false}finally{clearTimeout(timeout);historyController=null;historyLoading=false}}
async function currentRefreshLoop(){if(!document.hidden)await refreshCurrent();scheduleCurrent()}
async function historyRefreshLoop(){if(!document.hidden)await loadHistory(activeRange);scheduleHistory()}
...
function drawAll(){drawChart('chartT',1,'temp');drawChart('chartH',2,'hum');drawChart('chartP',3,'pres')}
document.querySelectorAll('button[data-range]').forEach(b=>b.addEventListener('click',async()=>{const range=b.dataset.range;if(range===activeRange&&historyLoading)return;abortFetch(historyController);historyLoading=false;clearTimeout(historyTimer);await loadHistory(range);scheduleHistory()}));
window.addEventListener('resize',()=>requestAnimationFrame(drawAll));
document.addEventListener('visibilitychange',async()=>{if(document.hidden){clearTimeout(currentTimer);clearTimeout(historyTimer);abortFetch(currentController);abortFetch(historyController);return}await refreshCurrent();await loadHistory(activeRange);scheduleCurrent();scheduleHistory()});
(async()=>{await refreshCurrent();await loadHistory('day');scheduleCurrent();scheduleHistory()})();
</script></body></html>
)HTML";
