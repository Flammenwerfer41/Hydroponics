/*
  ESP32 + BME280 Hydroponics Environment Logger v7.1
  ------------------------------------------------
  Features
  ...
  - Light event writes retry after transient LittleFS failures
...
bool appendLightEvent(uint32_t timestamp,bool on){
  ...
  if(!takeMutex(fsMutex,pdMS_TO_TICKS(3000))){Serial.println("Light event failed: filesystem mutex timeout.");return false;}
  ...
  f.flush();f.close();
  if(ok){newestLightEventSlot=(int32_t)next;if(validLightEventCount<MAX_LIGHT_EVENTS)validLightEventCount++;}
  giveMutex(fsMutex);
  if(ok)Serial.printf("Light event: %s at %s\n",on?"ON":"OFF",formatLocalTime(timestamp).c_str());
  return ok;
}
...
bool storeObservedLightState(uint32_t sampleTimestamp,bool on,uint32_t minutesToday){
  if(storedLightStateKnown&&lastStoredLightOn==on)return true;
  uint32_t eventTimestamp=sampleTimestamp;
  if(!storedLightStateKnown&&validLightEventCount==0&&on&&minutesToday>0){
  ...
  if(!appendLightEvent(eventTimestamp,on)){
    Serial.println("Light event write failed; will retry on next SwitchBot update.");
    return false;
  }
